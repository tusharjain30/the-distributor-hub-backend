...
import { addKeyAccountContactService, deleteKeyAccountContactService, getKeyAccountContactDetails, keyAccountContactListingService } from "../../../../services/api_v_1/distributor/keyAccount/contact.service.js";
...
export const keyAccountContactListingController = async (req, res) => {
    try {
        let response = RESPONSE;
        const user = req.user;
        ...
        if (user_info.status) {
            ...
            if (!distributor_info.status) {
                ...
                let account_info = await getDistributorkeyAccountDetails({ queryType: "distributorId_createdBy", _id: body.accountId, distributorId: body.distributorId, createdBy: body.createdBy });
                if (account_info.status) {
                    response = await keyAccountContactListingService(body);
                } else {
                    response = {
                        status: 0,
                        message: RESPONSE_MESSAGES.ACCOUNT_NOT_FOUND,
